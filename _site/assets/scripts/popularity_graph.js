...
class Graph {
  constructor(dataFile, eventHandler) {
    ...
    this.sigma = new sigma('popularity_graph');
...
function createTimeSlider(graph, eventHandler) {
	...
	const time_div_width = parseInt(d3.select('#popularity_time_slider').style("width"));
	...
	let time_slider = d3.sliderHorizontal()
		...
		.width(slider_width);
	let slider_div = d3.select('#popularity_time_slider').append('svg')
		.attr('width', "100%")
		.attr('height', "100%")
		.append('g')
